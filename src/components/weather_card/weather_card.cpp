...
void WeatherCard::init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_text_color(
        ...
        0);
    lv_obj_align(weather_icon, LV_ALIGN_TOP_MID, 0, 0);

    // Temperature content
    lv_obj_t *temp_cont = lv_obj_create(content);
    lv_obj_t *temp_icon = lv_img_create(temp_cont);
    // lv_img_set_src(temp_icon, &temp_symbol);
    lv_obj_align(temp_icon, LV_ALIGN_BOTTOM_LEFT, 0, 0);
    lv_obj_set_style_text_color(
        temp_icon,
        lv_color_hex(0x94A3B8),
        0);
    label_temp = lv_label_create(temp_cont);
    ...
    lv_obj_set_style_text_font(
        ...
        0);
    lv_obj_align(label_temp, LV_ALIGN_BOTTOM_RIGHT, 0, 0);
    lv_obj_align(temp_cont, LV_ALIGN_BOTTOM_LEFT, 0, 0);

    // Humidity content
    lv_obj_t *humidity_cont = lv_obj_create(content);
    lv_obj_t *humidity_icon = lv_img_create(humidity_cont);
    // lv_img_set_src(humidity_icon, &humidity_symbol);
    lv_obj_align(humidity_icon, LV_ALIGN_BOTTOM_LEFT, 0, 0);
    lv_obj_set_style_text_color(
        humidity_icon,
        lv_color_hex(0x94A3B8),
        0);
    label_humidity = lv_label_create(humidity_cont);
    ...
    lv_obj_align(humidity_cont, LV_ALIGN_BOTTOM_RIGHT, 0, 0);
};
